...
const stageBattleMovesExecutionMap: {
  [key: string]: (line: string, context: Context) => void;
} = {
  "": (line, context) => {
  ...
  moves: (line, context) => {
    ...
    if (line.match(/\[MOVE/)) {
      ...
      context.currMove.name = context.currMove.NAME = regexGrabStr(
        line,
        /MOVE_\w+/,
      );
    } else if (line.match(".effect")) {
      context.currMove.effect = Xtox(
        "EFFECT_",
        regexGrabStr(line, /EFFECT_\w+/),
      );
    } else if (line.match(".power")) {
      context.currMove.power = regexGrabNum(line, /(?<==)\d+/);
    } else if (line.match(".type")) {
      context.currMove.types.push(
        Xtox("TYPE_", regexGrabStr(line, /TYPE_\w+/)),
      );
      ...
      context.currMove.target = regexGrabStr(line, /(?<==)\w+/).replace(
        /^MOVE_TARGET_/,
        "",
      );
      ...
      context.currMove.flags = context.currMove.flags.concat(
        regexGrabStr(line, /(?<==)[^,]+/)
          .split("|")
          .map((x) => Xtox("FLAG_", x))
          .filter((x) => x !== "0"),
      ...
      context.currMove.splitModifier =
        SplitFlag[regexGrabStr(line, /(?<==)\w+/)];
    } else if (line.match(".split")) {
      context.currMove.split = regexGrabStr(line, /(?<==)\w+/).replace(
        /^SPLIT_/,
        "",
      );
...
const stageDescriptionExecutionMap: {
  [key: string]: (line: string, context: Context) => void;
} = {
  desc: (line, context) => {
    if (line.match("u8 s")) {
  ...
  descFourLine: (line, context) => {
    if (line.match("u8 s")) {
      if (context.currLongDesc.ptrDesc !== "") {
        context.LongDesc.set(
          context.currLongDesc.ptrDesc,
          context.currLongDesc,
        );
...
export function convertLegacyMove(legacyMove: Move): ProtoMove {
  ...
  const move = create(MoveSchema, {
    ...
    type2:
      legacyMove.types.length > 1
        ? Type[legacyMove.types[1].toUpperCase()]
        : Type.NONE,
    ...
    noKingsRock:
      !hasFlag("FLAG_KINGS_ROCK_AFFECTED") && legacyMove.split !== "STATUS",
  ...
  if (!move.effect && legacyMove.effect !== "Hit")
    console.log(legacyMove.effect);
  ...
  let noSheerForce =
    !hasFlag("FLAG_SHEER_FORCE_BOOST") &&
    legacyMove.split !== "STATUS" &&
    legacyMove.chance > 0 &&
    !sheerForceBannedEffects[MoveBehaviorSchema.value[move.effect].name];

  if (
    noSheerForce &&
    !(
      move.argument?.argument.case === "effect" &&
      sheerForceBannedMoveEffects[
        MoveEffectSchema.value[move.argument.argument.value.effect].name
      ]
    )
  ) {
  ...
  if (
    move.noKingsRock &&
    kingsRockBanned[MoveBehaviorSchema.value[move.effect].name]
  )
    move.noKingsRock = false;
...
function protoMoveToLegacyMove(
  move: ProtoMove,
  updatedMoveMapping: Map<MoveEnum, string>,
  updatedMoveEffectMapping: Map<MoveBehavior, string>,
): [string, Move] {
  const legacyMove: Move = {
    ...
    effect: Xtox(
      "EFFECT_",
      updatedMoveEffectMapping.get(move.effect) || "EFFECT_HIT",
    ),
    power: move.power,
    types: (move.type2
      ? [Type[move.type || 0] || "MYSTERY", Type[move.type2]]
      : [Type[move.type || 0] || "MYSTERY"]
    ).map((it) => Xtox("TYPE_", it)),
    acc: move.accuracy,
    pp: move.pp,
    ...
    flags: [],
    split: move.splitModifier
      ? SplitFlag[move.splitModifier]
      : MoveSplit[move.split || 0],
    splitModifier: SplitFlag.USE_BASE_SPLIT,
    argument: "",
  ...
  for (const field of MoveSchema.fields) {
    if (getOption(field, is_flag) && move[field.name]) {
      if (field.enum) {
        legacyMove.flags.push(
          getOption(field.enum.value[move[field.name]], enum_name),
        );
      } else {
...
export function getMoves(ROOT_PRJ: string, gameData: GameData) {
  ...
  gameData.moveMap = new Map(gameData.moveList.moves.map(it => [it.id, it]))
  const moveEnumMapping = gameData.moveEnumMap;
  const moveEffectMapping = getUpdatedMoveEffectMapping(ROOT_PRJ);
  gameData.moves = new Map(
    gameData.moveList.moves.map((it) =>
      protoMoveToLegacyMove(it, moveEnumMapping, moveEffectMapping),
    ),
  );
  gameData.tutors = gameData.moveList.moves
    .filter((it) => it.tutor)
    .map((it) => gameData.moveEnumMap.get(it.id)!!)
    .sort();
  gameData.universalTutors = gameData.moveList.moves
    .filter((it) => it.tutor === TutorType.TUTOR_UNIVERSAL_STATUS)
    .map((it) => gameData.moveEnumMap.get(it.id)!!);
  gameData.universalAttackTutors = gameData.moveList.moves
    .filter((it) => it.tutor === TutorType.TUTOR_UNIVERSAL_ATTACK)
    .map((it) => gameData.moveEnumMap.get(it.id)!!);
  gameData.universalGenderedTutors = gameData.moveList.moves
    .filter((it) => it.tutor === TutorType.TUTOR_UNIVERSAL_STATUS_GENDERED)
    .map((it) => gameData.moveEnumMap.get(it.id)!!);
}

export function getLegacyMoves(
  ROOT_PRJ: string,
  optionsGlobal_h: FileDataOptions,
  gameData: GameData,
): Promise<void> {
  return new Promise((resolve: () => void, reject) => {
    getFileData(
      join(ROOT_PRJ, "include/constants/battle_config.h"),
      optionsGlobal_h,
    )
      .then((battle_config) => {
        const optionsBattle = {
